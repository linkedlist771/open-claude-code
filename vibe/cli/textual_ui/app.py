...
class VibeApp(App):
    ...
    async def _show_status(self) -> None:
        ...
        status_text = f"""## Claude Vibe Status

- **Version**: {__version__}
- **Model**: {self.config.active_model} ({active_model.name if "active_model" in dir() else "N/A"})
- **Provider**: {provider.name if "provider" in dir() else "N/A"}
- **API Key**: {api_key_env}
